...
已完成临时模块加载、3 台及单台从站识别。用户已确认单台的电机与反馈线接好，PREOP 下的一次 SDO 故障复位尚未清除 `0x738A`。现已确认主站类型为标准 CiA402 的 `0`，读出编码器类型值 `5` 和 17/16 位分辨率，并通过等待字典完成避免诊断邮箱并发。下一步结合实物型号/固件核对编码器参数及 PREOP 下控制字处理方式，整理明确的 PDO 初始化。随后在禁止使能状态验证周期通信与 DC，再验证反馈、急停链、停止和使能状态机，最后开展限幅单轴运动。
