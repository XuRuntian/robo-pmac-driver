...
def load_omega_teleop_config(path: str | Path = "config/omega_teleop.yaml") -> OmegaTeleopConfig:
    ...
    def load_axis(name: str) -> OmegaAxisConfig:
        section = raw.get(name, {})
        return OmegaAxisConfig(
            map=_map(section.get("map", "xyz"), f"{name}.map"),
            signs=_triple(section.get("signs", [1, 1, 1]), f"{name}.signs", signs=True),
            gain=_triple(section.get("gain", [1.0, 1.0, 1.0]), f"{name}.gain"),
        )

    return OmegaTeleopConfig(load_axis("translation"), load_axis("rotation"))
