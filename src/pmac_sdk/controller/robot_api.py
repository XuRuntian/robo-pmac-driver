...
class PMACRobotController:
    ...
    def move_joints(self, target_pulses: list):
        """核心底层：只下发原版的地址 0 和 地址 100"""
        # 1. 写入 5 个电机的目标坐标 (对应原代码的 address=0)
        self.modbus.write_int32_array(address=0, values=target_pulses)
        
        # 2. 扣动扳机 (对应原代码的 address=100)
        self.modbus.write_int32_array(address=100, values=[1])

    def move_single_joint_angle(self, joint_idx: int, angle: float):
        """按照你的原版逻辑换算角度"""
